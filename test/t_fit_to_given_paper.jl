...
begin
    ...
    background("gold4")
    setcolor("gold")
    RouteMap.draw_grid(grid, ymax, xmax, grid, grid, 0.5) # draw_utm_grid is the exported form.
...
# and taking a snapshot with a label overlay just displays the labels that happens to be close to origin. 
...
@test world_to_paper_factor(model) ≈ 0.006230213491354818
...
plot_legs_in_model_space_and_push_labels_to_model!(model, legs)
# Show just what's currently on the model canvas in ink extent
snap() #18
...
# Show just what's currently on the model canvas in ink extent
snap() #19
@test abs(world_to_paper_factor(model) - 0.006227938491499979) < 0.00001
...
legs = [leg];
plot_legs_in_model_space_and_push_labels_to_model!(model, legs) # This plots the path geometry and updates model's `label` collection
